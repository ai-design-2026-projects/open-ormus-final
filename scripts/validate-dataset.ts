...
const VARYING_AXES: VaryingAxis[] = [
  "speechPatterns",
  "copingStyle",
  "fears",
  "goals",
...
function validateSchema(c: unknown, idx: number): string[] {
  ...
  if (
    ...
    !VARYING_AXES.includes(char.varyingAxis as VaryingAxis)
  )
    errors.push(`${prefix} invalid varyingAxis: ${char.varyingAxis}`);

  // Field length constraints
  if (
    isStringArray(char.personalityTraits) &&
    (char.personalityTraits.length < 4 || char.personalityTraits.length > 6)
  )
    errors.push(
      `${prefix} personalityTraits must have 4–6 items, got ${char.personalityTraits.length}`,
    );
  if (
    isStringArray(char.speechPatterns) &&
    (char.speechPatterns.length < 3 || char.speechPatterns.length > 4)
  )
    errors.push(
      `${prefix} speechPatterns must have 3–4 items, got ${char.speechPatterns.length}`,
    );
  if (
    isStringArray(char.values) &&
    (char.values.length < 3 || char.values.length > 4)
  )
    errors.push(
      `${prefix} values must have 3–4 items, got ${char.values.length}`,
    );
  if (
    isStringArray(char.fears) &&
    (char.fears.length < 2 || char.fears.length > 3)
  )
    errors.push(
      `${prefix} fears must have 2–3 items, got ${char.fears.length}`,
    );
  if (
    isStringArray(char.goals) &&
    (char.goals.length < 2 || char.goals.length > 3)
  )
    errors.push(
      `${prefix} goals must have 2–3 items, got ${char.goals.length}`,
    );
  if (
    isStringArray(char.notableQuotes) &&
    (char.notableQuotes.length < 2 || char.notableQuotes.length > 3)
  )
    errors.push(
      `${prefix} notableQuotes must have 2–3 items, got ${char.notableQuotes.length}`,
    );
  if (
    isStringArray(char.abilities) &&
    (char.abilities.length < 3 || char.abilities.length > 4)
  )
    errors.push(
      `${prefix} abilities must have 3–4 items, got ${char.abilities.length}`,
    );
  if (
    isStringArray(char.copingStyle) &&
    (char.copingStyle.length < 2 || char.copingStyle.length > 3)
  )
    errors.push(
      `${prefix} copingStyle must have 2–3 items, got ${char.copingStyle.length}`,
    );
...
function validateSemantics(chars: Character[]): string[] {
  ...
  for (const c of chars) {
    if (c.difficultyTier === "distinctive") {
      if (c.similarTo !== null)
        errors.push(`${c.id}: distinctive tier must have similarTo: null`);
      if (c.varyingAxis !== null)
        errors.push(`${c.id}: distinctive tier must have varyingAxis: null`);
    }
    if (c.difficultyTier === "similar_pair") {
      if (!c.similarTo)
        errors.push(`${c.id}: similar_pair must have non-null similarTo`);
      if (!c.varyingAxis)
        errors.push(`${c.id}: similar_pair must have non-null varyingAxis`);
    }
    if (c.similarTo && !byId.has(c.similarTo))
      errors.push(
        `${c.id}: similarTo references non-existent id ${c.similarTo}`,
      );
    if (c.similarTo) {
      const partner = byId.get(c.similarTo);
      if (partner && partner.similarTo !== c.id)
        errors.push(
          `${c.id}: similarTo is not bidirectional (partner.similarTo = ${partner.similarTo})`,
        );
    }
    if (c.similarTo && c.varyingAxis) {
      ...
      if (partner && partner.varyingAxis !== c.varyingAxis)
        errors.push(
          `${c.id}: varyingAxis mismatch with partner ${c.similarTo}`,
        );
    }
  ...
  if (distinctive.length !== 8)
    errors.push(`Expected 8 distinctive characters, got ${distinctive.length}`);
  if (pairs.length !== 8)
    errors.push(`Expected 8 similar_pair characters, got ${pairs.length}`);
  ...
  for (const c of pairs) {
    ...
    for (const field of ARRAY_FIELDS) {
      ...
      if (aVal !== bVal)
        errors.push(
          `Pair ${c.id}↔${c.similarTo}: field "${field}" should be identical but differs`,
        );
    }
    ...
    const QUOTE_EXEMPT_AXES: VaryingAxis[] = [
      "speechPatterns",
      "fears",
      "goals",
    ];
    if (!QUOTE_EXEMPT_AXES.includes(c.varyingAxis as VaryingAxis)) {
      ...
      if (aQ !== bQ)
        errors.push(
          `Pair ${c.id}↔${c.similarTo}: field "notableQuotes" should be identical but differs`,
        );
    }
    if (c.backstory !== partner.backstory)
      console.warn(
        `  WARN: ${c.id}↔${c.similarTo}: backstory differs (may be intentional for pronoun agreement)`,
      );
  }
