...
const raw = JSON.parse(
  readFileSync(
    resolve(import.meta.dir, "../evaluation/dataset/scenarios.json"),
    "utf-8",
  ),
) as unknown;
...
for (let i = 0; i < raw.length; i++) {
  ...
  check(
    isStringNonEmpty(s["initial_prompt"]),
    `initial_prompt is non-empty string`,
  );

  const validDifficulty = ["baseline", "moderate", "high"].includes(
    s["difficulty_level"] as string,
  );
  check(
    validDifficulty,
    `difficulty_level is valid (got "${s["difficulty_level"]}")`,
  );

  const validSocialCtx = SOCIAL_CONTEXTS.includes(
    s["social_context"] as SocialContext,
  );
  check(
    validSocialCtx,
    `social_context is valid (got "${s["social_context"]}")`,
  );

  const validPressure = PRESSURE_SOURCES.includes(
    s["pressure_source"] as PressureSource,
  );
  check(
    validPressure,
    `pressure_source is valid (got "${s["pressure_source"]}")`,
  );
  ...
  if (s["difficulty_level"] === "baseline") {
    check(
      axes.length === 0,
      `baseline scenario has 0 stress_axes (got ${axes.length})`,
    );
  } else {
    check(
      axes.length >= 1 && axes.length <= 4,
      `stress_axes count 1–4 (got ${axes.length})`,
    );
  }
  ...
  for (const axis of axes) {
    ...
    if (validAxis)
      axisCounts.set(axis as string, (axisCounts.get(axis as string) ?? 0) + 1);
  }

  if (validDifficulty)
    difficultyCounts[s["difficulty_level"] as DifficultyLevel]++;
}
...
check(
  seenCells.size === 32,
  `all 32 coverage cells filled (got ${seenCells.size})`,
);
